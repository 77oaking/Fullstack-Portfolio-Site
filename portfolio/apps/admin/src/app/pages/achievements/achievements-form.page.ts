  ...
  template: `
    <div class="page">
      <div class="page-header">
        <div><h1 class="page-title">{{ id() ? 'Edit achievement' : 'New achievement' }}</h1></div>
        <a mat-stroked-button routerLink="/achievements"><mat-icon>arrow_back</mat-icon> Back</a>
      </div>
      <form [formGroup]="form" (ngSubmit)="save()" class="card">
        <div class="row">
          ...
          <mat-form-field appearance="outline">
            ...
            <input matInput formControlName="date" placeholder="2024-08" />
          </mat-form-field>
          <mat-form-field appearance="outline">
...
export class AchievementsFormPage implements OnInit {
  ...
  save(): void {
    ...
    obs.subscribe({
      next: () => { this.saving.set(false); this.snack.open('Saved', 'Dismiss', { duration: 2500 }); this.router.navigate(['/achievements']); },
